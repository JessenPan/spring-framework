...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
    ...
    private <T> T doGetBean(final String name, final Class<T> requiredType, final Object[] args, boolean typeCheckOnly)
            throws BeansException {
        //转换名称,把别名转换为正式名称
        final String beanName = transformedBeanName(name);
        Object bean;

        //检测缓存中或者实例工厂中是否有对应的实例
        ...
        if (sharedInstance != null && args == null) {
            ...
            //如果在prototype创建类型下出现循环创建的情况,则直接抛出创建异常
            ...
            //如果不是做类型检查，而是创建，则这里需要做标记
            if (!typeCheckOnly) {
                markBeanAsCreated(beanName);
            }

            try {
                //将表示本地xml配置的genericBeanDefinition和父beanDefinition进行合并,形成
                //rootBeanDefinition
                ...
                //实例化依赖的bean之后便可以实例化我们需要的bean本身了
                //单例模式的创建
                if (mbd.isSingleton()) {
     ...
     */
    private void destroyBean(String beanName, Object beanInstance, RootBeanDefinition mbd) {
     ...
     */
    private String originalBeanName(String name) {
        String beanName = transformedBeanName(name);
     ...
     */
    private void checkMergedBeanDefinition(RootBeanDefinition mbd, String beanName, Object[] args)
            throws BeanDefinitionStoreException {
        ...
        if (args != null && !mbd.isPrototype()) {
            throw new BeanDefinitionStoreException(mbd.getResourceDescription(), beanName, "Can only specify arguments for the getBean method when referring to a prototype bean definition");
        }
    }
     ...
     * @param beanName the name of the bean
     */
    private void cleanupAfterBeanCreationFailure(String beanName) {
     ...
     * <br/>
     * 获取指定bean实例对应的对象,可以是beanInstance实例它自己。也可能是它创建出来的实例对象,在类型为FactoryBean的情况会出现
     *
     * @param beanInstance the shared bean instance
     * @param name         name that may include factory dereference prefix
     * @param beanName     the canonical bean name
     * @param mbd          the merged bean definition
     * @return the object to expose for the bean, 根据指定的bean暴露的object对象
     */
    private Object getObjectForBeanInstance(Object beanInstance, String name, String beanName, RootBeanDefinition mbd) {

        //如果实例不是factoryBean,同时获取的bean的名称又是要获取工厂,则抛异常
        if (BeanFactoryUtils.isFactoryDereference(name) && !(beanInstance instanceof FactoryBean)) {
            throw new BeanIsNotAFactoryException(transformedBeanName(name), beanInstance.getClass());
        }

        //1.如果beanIns不是factoryBean实例,则直接返回
        //2.如果beanIns是 factoryBean实例,同时外部接口需要的也是工厂引用,则返回对应的工厂引用
        if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
            return beanInstance;
        }

        //下面是beanIns是factoryBean，但是取得name不是工厂引用的情况
        Object object = null;
        if (mbd == null) {
    ...
    private boolean requiresDestruction(Object bean, RootBeanDefinition mbd) {
        return (bean != null && (DisposableBeanAdapter.hasDestroyMethod(bean, mbd) || hasDestructionAwareBeanPostProcessors()));
    }
