...
/**
 * beanDefinition接口的基础抽象类,主要给具体实现类继承使用.
 ...
 * <p>
 * <p>此类主要是把GenericBeanDefinition,RootBeanDefinition以及ChildBeanDefinition的共性给抽象出来
 * <p>The autowire constants match the ones defined in the
 * {@link org.springframework.beans.factory.config.AutowireCapableBeanFactory}
 * interface.
 * <p>
 * 此类的autowire属性和AutowireCapableBeanFactory中定义的一直
 *
 * @author Rod Johnson
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
    ...
    //此类是否作为autowire查找的候选类,为false的话，即使名称或者类型匹配也不会被其他类自动注入
    //但是此类的autowireMode可以设置为自动注入其他类
    private boolean autowireCandidate = true;
    private boolean primary = false;
    private boolean nonPublicAccessAllowed = true;

    //是否已宽松的形式解析构造函数,对于有相同数目的构造函数入参的bean定义解析会报错
    private boolean lenientConstructorResolution = true;

    //记录构造函数在xml中的配置,即constructor-arg
    private ConstructorArgumentValues constructorArgumentValues;

    //bean属性注入,即<property>培训
    private MutablePropertyValues propertyValues;

    //方法重写配置
    private MethodOverrides methodOverrides = new MethodOverrides();

    //创建本类的工厂方法,对应factory-bean配置属性
    private String factoryBeanName;

    //当一个类既包含本身功能，同时包含工厂方法时指定对应的factory-method配置属性 
    private String factoryMethodName;

    //当依赖的属性设置完成之后,执行的初始化方法
    private String initMethodName;

    //bean生命周期结束之后,执行的销毁方法.对应xml配置的属性destroy-method
    private String destroyMethodName;

    //是否执行init方法,此属性是由程序设置
    private boolean enforceInitMethod = true;

    //是否执行destory-method,程序设置
    private boolean enforceDestroyMethod = true;

    //是否是用户定义的而不是程序本身定义的,创建AOP的时候为true,因为是程序动态生成的
    private boolean synthetic = false;

    //此定义的beanDefinition的角色,application表示程序中用户配置的bean
    private int role = BeanDefinition.ROLE_APPLICATION;

    //beanDefinition的描述信息
    private String description;

    //定义这个beanDefinition的资源文件
    private Resource resource;


    /**
     * Create a new AbstractBeanDefinition with default settings.
     * 默认配置创建一个AbstractBeanDefinition
     */
     ...
     * <p>
     * 使用指定的constructorArgs和MutablePropValues创建AbstractBeanDefinition
     */
     ...
     * bean definition.
     * <p>
     * 基于另外一个beanDefinition进行深拷贝创建一个新的BeanDefinition
     *
     * @param original the original bean definition to copy from
     * @deprecated since Spring 2.5, in favor of {@link #AbstractBeanDefinition(BeanDefinition)}
     * 2.5之后，丢弃此方法用了AbstractBeanDefinition(BeanDefinition)
     * //tips 在函数签名里，优先使用接口，而不是实现类,面向接口编程
     */
    @Deprecated
    ...
    protected AbstractBeanDefinition(BeanDefinition original) {
        ...
        //这里设置值之前,又new了一个出来,主要作用是进行防御性拷贝
        setConstructorArgumentValues(new ConstructorArgumentValues(original.getConstructorArgumentValues()));
        //tips 对于易变内容，考虑使用防御性拷贝
     ...
     * <p>
     * 用public修饰符修饰的clone方法
     * 此方法的实现是委托给#cloneBeanDefinition的
     *
     * @see Object#clone()
     */
    @Override
     ...
     * <p>
     * 从此beanDefinition克隆出一个新的beanDefinition.
     * 此方法为抽象方法，具体实现由子类实现
     *
    ...
    //tips 对于简单值类型java类，最好总是重写equals,hashcode,toString
    @Override
