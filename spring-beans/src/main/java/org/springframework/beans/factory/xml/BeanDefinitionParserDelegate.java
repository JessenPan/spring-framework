 ...
 * <br/>
 * 有状态的代理类,用来解析xml里的bean definitions.
 * 被使用在主解析器以及任何扩展，如beanDefinitionParser和BeanDefinitionDecorator
 *
 * @author Rob Harrop
...
public class BeanDefinitionParserDelegate {
    ...
    private static final Log LOGGER = LogFactory.getLog(BeanDefinitionParserDelegate.class);
    ...
    private static final String AUTOWIRE_NO_VALUE = "no";
    ...
    private static final String AUTOWIRE_CANDIDATE_ATTRIBUTE = "autowire-candidate";
    ...
    private static final String QUALIFIER_ELEMENT = "qualifier";

    private static final String QUALIFIER_ATTRIBUTE_ELEMENT = "attribute";

    private static final String DEFAULT_LAZY_INIT_ATTRIBUTE = "default-lazy-init";

    private static final String DEFAULT_MERGE_ATTRIBUTE = "default-merge";
    ...
    private static final String DEFAULT_AUTOWIRE_CANDIDATES_ATTRIBUTE = "default-autowire-candidates";

    private static final String DEFAULT_INIT_METHOD_ATTRIBUTE = "default-init-method";

    private static final String DEFAULT_DESTROY_METHOD_ATTRIBUTE = "default-destroy-method";
     ...
     * <br/>
     * 保持所有使用过的bean名称,这样我们就能够强制每一个bean元素的名称唯一性
     * <br/>
     * 重复的bean名称和id不可能在同级别的bean嵌套里出现，但可能在扩级别的定义中出现重复
     */
     ...
     * <br/>
     * 根据指定的xmlReaderContext创建BeanDefinitionParserDelegate
     */
     ...
     * <br/>
     * 通过调用SourceExtractor从指定的element中获取源的元数据
     */
     ...
     */
    private boolean parseMergeAttribute(Element collectionElement) {
    ...
    private BeanDefinition parseCustomElement(Element ele, BeanDefinition containingBd) {
        //获取自定义元素对应的命名空间名称
    ...
    private BeanDefinitionHolder decorateBeanDefinitionIfRequired(Element ele, BeanDefinitionHolder definitionHolder, BeanDefinition containingBd) {
    ...
    private BeanDefinitionHolder decorateIfRequired(Node node, BeanDefinitionHolder originalDef, BeanDefinition containingBd) {
        ...
        if (!isDefaultNamespace(namespaceUri)) {
            ...
            if (handler != null) {
                ...
                //如果命名空间不为空，同时命名空间以spring官方的uri开头则抛异常
     ...
     * <br/>
     * 获取指定node的命名URI.默认的实现使用了Node#getNamespaceURI实现.
     * 子类可以重写此默认实现来提供一个不同的命名空间标识机制
     *
     * @param node the node
     */
     ...
     * 判断指定的命名空间是否为默认命名空间
     *
     * @param namespaceUri 指定的命名空间
     * @return true, 默认命名空间.false自定义xml命名空间
     */
    ...
    /**
     * 判断指定的xml的node结点是否属于默认命名空间
     *
     * @param node xml的结点
     * @return true, 默认命名空间.false, 自定义xml命名空间
     */
