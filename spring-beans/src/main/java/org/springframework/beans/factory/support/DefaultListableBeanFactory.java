...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
    ...
    private static final Log LOGGER = LogFactory.getLog(DefaultListableBeanFactory.class);
     ...
     * <br/>
     * 所有的注册的beanDefinition集合，以注册的顺序存储
     */
     ...
     * <br/>
     * 对于相同名称的beanDefinition定义是否可以重复定义
     */
     ...
     * <br/>
     * 是否beanFactory的配置已经冻结了，即是否可以缓存beanDefinition的元数据了
     */
     ...
     * <br/>
     * 在冻结了配置的情况下，缓存beanDefinition的名称
     */
    ...
    public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
        ...
        if (beanDefinition instanceof AbstractBeanDefinition) {
            //如果是AbstractBeanDefinition实例,先做校验
            try {
        ...
        //对于全局共享变量，同时可能并发访问进行并发控制
        synchronized (this.beanDefinitionMap) {
            oldBeanDefinition = this.beanDefinitionMap.get(beanName);
            if (oldBeanDefinition != null) {
                //已经存在相同名称的beanDefinition定义
                if (!this.allowBeanDefinitionOverriding) {
                    //不允许重复定义相同名称的beanDefinition,抛出异常
                    throw new BeanDefinitionStoreException(beanDefinition.getResourceDescription(), beanName, "Cannot register bean definition [" + beanDefinition + "] for bean '" + beanName + "': There is already [" + oldBeanDefinition + "] bound.");
                }
            } else {
                //新的beanDefinition定义,添加到集合里
                this.beanDefinitionNames.add(beanName);
                //缓存的冻结数据清空
                this.frozenBeanDefinitionNames = null;
            }
            //添加beanName到beanDefinition的map
            this.beanDefinitionMap.put(beanName, beanDefinition);
        }

        //之前已经定义的beanDefinition不为空,或者单例对象集合包含此beanName
        //则重置所有缓存
    ...
    private Object doResolveDependency(DependencyDescriptor descriptor, Class<?> type, String beanName,
                                       Set<String> autowiredBeanNames, TypeConverter typeConverter) throws BeansException {
